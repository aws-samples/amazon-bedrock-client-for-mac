...
final class MCPServerTests: XCTestCase {
    override func setUpWithError() throws {
        try XCTSkipUnless(ProcessInfo.processInfo.environment["BEDROCK_LIVE_NETWORK_TESTS"] == "1",
                          "Public MCP endpoint diagnostics are opt-in. Local MCP integration tests run without network credentials.")
    }
    ...
    private static func testServer(_ server: MCPServerTestData) async -> TestResult {
