...
final class MCPIntegrationTests: XCTestCase {
    ...
    private func makeManager() throws -> MCPClientManager {
        ...
        let manager = MCPClientManager(configurationDirectory: root, preferences: preferences, autoStart: false)
        // The test host disables shared-server startup with a launch argument.
        // NSArgumentDomain outranks even a named suite. Enable only this
        ...
        while Date() < deadline {
            ...
            for name in names {
                if case .failed(let error) = manager.connectionStatus[name] {
                    ...
                    throw LocalOperationError.unavailable(error)
                }
            }
            try await Task.sleep(for: .milliseconds(20))
        }
        XCTFail("MCP connection deadline exceeded")
        throw LocalOperationError.unavailable("Connection deadline exceeded")
    }
