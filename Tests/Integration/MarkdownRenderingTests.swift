...
final class MarkdownRenderingTests: XCTestCase {
    ...
    func testReadOnlySelectionMenuKeepsOnlyCopyAndSelectAll() {
        ...
        let menu = WorkbenchTextMenu.make(for: view)
        ...
        XCTAssertFalse(WorkbenchTextMenu.make(for: view).items[0].isEnabled)
    }
    ...
    func testComposerContextMenuKeepsEditingAndUsesTheActualPasteHandler() {
        ...
        let items = WorkbenchTextMenu.make(for: view).items.filter { !$0.isSeparatorItem }
        XCTAssertEqual(items.map(\.title), ["Cut", "Copy", "Paste", "Select All"])
        XCTAssertEqual(items[2].action, #selector(MyTextView.paste(_:)))
    ...
    func testLongReplyReflowsAndRetainsHeightOnRepeatedLayout() async {
        ...
        await MainActor.run {
            ...
            let host = NSHostingView(rootView: WorkbenchMarkdown(rows: rows, fontSize: 14, highlights: []).frame(width: 720))
            ...
            host.rootView = WorkbenchMarkdown(rows: rows, fontSize: 14, highlights: []).frame(width: 420)
            let narrow = host.fittingSize
            XCTAssertGreaterThan(narrow.height, wide.height)
            XCTAssertEqual(narrow.width, 420, accuracy: 1)
            host.rootView = WorkbenchMarkdown(rows: rows, fontSize: 14, highlights: []).frame(width: 720)
            XCTAssertEqual(host.fittingSize.height, wide.height, accuracy: 1)
