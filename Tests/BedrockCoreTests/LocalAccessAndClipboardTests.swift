...
final class LocalAccessAndClipboardTests: XCTestCase {
    ...
    func testBundledSkillUpgradePreservesUserEdits() throws {
        ...
        """
        let upgraded = try XCTUnwrap(SkillDefinition.bundledUpgrade(id: "code-review", source: original))
        XCTAssertTrue(upgraded.contains("Absolute and ~/ paths"))
        XCTAssertFalse(upgraded.contains("ask the user to select a folder"))
        XCTAssertNil(SkillDefinition.bundledUpgrade(id: "code-review", source: original + "\nMy custom rule."))
        XCTAssertNil(SkillDefinition.bundledUpgrade(id: "my-review", source: original))
    }
    ...
    func testNewPreferencesPermitAllToolsAndLocalPathsWithoutApproval() throws {
        ...
        XCTAssertEqual(preferences.enabledTools, Set(BuiltInTool.allCases))
        for tool in BuiltInTool.allCases { XCTAssertFalse(preferences.approvalMode.requiresApproval(tool: tool)) }
    ...
    func testOldDefaultMigratesWhileConfiguredRestrictionsAndDraftsRemain() throws {
        ...
        let folder = LegacyProject(name: "Legacy", path: "/tmp/legacy")
        ...
        var migrated = try JSONDecoder().decode(AppDataSnapshot.self, from: JSONEncoder().encode(state))
