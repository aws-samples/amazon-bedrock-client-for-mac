...
final class CoreBehaviorTests: XCTestCase {
    ...
    func testStateRoundTripPreservesIndependentThreadDraftsAndSelections() throws {
        ...
        let file = JSONFile<AppDataSnapshot>(url: root.appendingPathComponent("state.json"))
        var state = AppDataSnapshot()
        let project = LegacyProject(name: "Fixture", path: root.path)
        state.projects = [project]
        state.threads["one"] = ThreadMetadata(draft: "안녕하세요", projectID: project.id, skillIDs: ["code-review"])
        state.threads["two"] = ThreadMetadata(draft: "different draft", pinnedAt: Date(), archived: true)
    ...
    func testCorruptStateIsNotTreatedAsEmptyAndRemainsUntouched() throws {
        ...
        XCTAssertThrowsError(try JSONFile<AppDataSnapshot>(url: url).load())
    ...
    func testSkillParsesMultilineDescriptionNestedRequirementsAndLists() throws {
        ...
        """
        let skill = try SkillDefinition.parse(source, id: "review", url: URL(fileURLWithPath: "/tmp/SKILL.md"))
    ...
    func testSkillLiteralBlockAndMalformedFrontmatter() throws {
        ...
        XCTAssertThrowsError(try SkillDefinition.parse("---\nname: broken\nbody", id: "bad", url: skill.url))
        XCTAssertThrowsError(try SkillDefinition.parse("---\nname: empty\n---\n", id: "bad", url: skill.url))
    }

    func testOnlySelectedEnabledSkillsEnterContextWithoutMutatingTheirFiles() throws {
        let skills = try SkillDefinition.bundled.map { try SkillDefinition.parse($0.source, id: $0.id, url: URL(fileURLWithPath: "/tmp/\($0.id)/SKILL.md")) }
        let context = try SkillDefinition.context(for: ["code-review", "document-analysis"], skills: skills, enabled: ["document-analysis": false])
        ...
        XCTAssertThrowsError(try SkillDefinition.context(for: ["code-review"], skills: skills, enabled: [:], limit: 10))
    ...
    func testProfileRemainsAnUpperBoundAndApprovalModes() {
        ...
        XCTAssertFalse(preferences.enabledTools.contains(.runCommand))
        XCTAssertFalse(ToolApprovalMode.askForChanges.requiresApproval(tool: .readFile))
        XCTAssertTrue(ToolApprovalMode.askForChanges.requiresApproval(tool: .writeFile))
        XCTAssertTrue(ToolApprovalMode.askForChanges.requiresApproval(tool: nil))
        XCTAssertTrue(ToolApprovalMode.askAlways.requiresApproval(tool: .readFile))
    }
    ...
    func testChatOnlyProfileDoesNotAdvertiseStatusTools() {
        XCTAssertTrue(ToolProfile.chat.tools.isEmpty)
        XCTAssertFalse(ToolProfile.readOnly.tools.contains(.sessionStatus))
        XCTAssertFalse(ToolProfile.developer.tools.contains(.sessionStatus))
        var preferences = AppPreferences()
    ...
    func testScheduleCalculationsDoNotReplayMissedIntervals() throws {
        ...
        var automation = AutomationDefinition(name: "Demo", prompt: "Hello", modelID: "model")
    ...
    func testDailyScheduleUsesCalendarAcrossDaylightSaving() throws {
        ...
        var automation = AutomationDefinition(name: "Daily", prompt: "Hello", modelID: "model")
    ...
    func testSettingsAndDemosHaveUniqueStableDestinations() {
        XCTAssertEqual(Set(SettingsItem.all.map(\.id)).count, SettingsItem.all.count)
        XCTAssertEqual(Set(DemoPreset.builtIns.map(\.id)).count, DemoPreset.builtIns.count)
        XCTAssertEqual(Set(SettingsItem.all.map(\.pane)), Set(SettingsPane.allCases))
        XCTAssertTrue(SettingsItem.all.contains { $0.id == "approval" && $0.matches("permission") })
        for tool in BuiltInTool.allCases {
            XCTAssertTrue(SettingsItem.all.contains { $0.id == tool.rawValue && $0.pane == .tools })
        }
    }
