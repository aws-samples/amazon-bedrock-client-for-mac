...
final class SkillLibraryTests: XCTestCase {
    ...
    func testLoadReportsBrokenOversizedAndLinkedSkillsWithoutLosingValidEntries() throws {
        ...
        try write(String(repeating: "x", count: SkillLibrary.instructionLimit + 1), to: root.appendingPathComponent("large/SKILL.md"))
        ...
        let result = try SkillLibrary.load(root)
    ...
    func testImportExportPreservesReferencesBinaryBytesAndExecutablePermission() throws {
        ...
        let id = try SkillLibrary.importPackage(from: original, into: library)
        XCTAssertEqual(id, "original")
        let skill = try XCTUnwrap(try SkillLibrary.load(library).skills.first)
        XCTAssertEqual(try SkillLibrary.references(for: skill).map(\.path).sorted(),
                       ["references/binary.dat", "references/example.txt", "scripts/example.sh"])
        let exported = root.appendingPathComponent("Exported")
        try SkillLibrary.export(skill, to: exported)
        ...
        XCTAssertThrowsError(try SkillLibrary.importPackage(from: original, into: library))
        XCTAssertThrowsError(try SkillLibrary.export(skill, to: exported))
        XCTAssertEqual(try SkillLibrary.load(library).skills.count, 1)
    }
    ...
    func testImportRejectsReferenceSymlinksAndLeavesNoPartialPackage() throws {
        ...
        let library = root.appendingPathComponent("Library")
        XCTAssertThrowsError(try SkillLibrary.importPackage(from: sourceFolder, into: library))
    ...
    func testSaveValidatesBeforeReplacingExistingInstructions() throws {
        ...
        try SkillLibrary.save(id: "fixture", source: source, existingURL: nil, directory: root)
        XCTAssertThrowsError(try SkillLibrary.save(id: "fixture", source: "---\nbroken", existingURL: url, directory: root))
        XCTAssertThrowsError(try SkillLibrary.save(id: "../escape", source: source, existingURL: nil, directory: root))
        XCTAssertThrowsError(try SkillLibrary.save(id: "fixture", source: source + "\nreplacement", existingURL: nil, directory: root))
    ...
    func testAvailabilityAndAppliedContextUseTheSameSnapshot() throws {
        ...
        XCTAssertThrowsError(try SkillDefinition.context(for: ["missing"], skills: result.skills, enabled: [:], unavailable: result.unavailable))
        XCTAssertFalse(try SkillDefinition.context(for: ["missing"], skills: result.skills, enabled: [:], unavailable: [:]).isEmpty)
    }
    ...
    func testCancelledImportCannotPublishAPartialSkill() async throws {
        ...
        let task = Task.detached {
            ...
            return try SkillLibrary.importPackage(from: sourceFolder, into: library)
        }
