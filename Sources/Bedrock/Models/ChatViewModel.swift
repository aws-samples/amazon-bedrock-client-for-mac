...
class ChatViewModel: ObservableObject {
    ...
    init(chatId: String, backendModel: BackendModel, chatManager: ChatManager = .shared, sharedMediaDataSource: SharedMediaDataSource) {
        ...
        self.userInput = WorkbenchStore.shared.thread(chatId).draft
    }
    ...
    private func setupBindings() {
        ...
        $userInput.dropFirst().sink { [weak self] text in
            ...
            WorkbenchStore.shared.updateDraft(self.chatId, text: text)
        }.store(in: &cancellables)
    ...
    func loadInitialData() {
        ...
        if let cached = chatManager.cachedMessages(for: chatId), cached.isEmpty,
           let url = try? ConversationOutboxFile.url(threadID: chatId, directory: WorkbenchStore.shared.directory),
           let draftURL = try? ConversationAttachmentDraftFile.url(threadID: chatId, directory: WorkbenchStore.shared.directory),
    ...
    private func loadAttachmentDraft() async {
        ...
        do {
            let url = try ConversationAttachmentDraftFile.url(threadID: chatId, directory: WorkbenchStore.shared.directory)
            ...
            attachmentsRestored = true
            WorkbenchStore.shared.updateThread(chatId) { $0.hasDraftAttachments = !sharedMediaDataSource.isEmpty }
        } catch {
            attachmentsRestored = true
            canWriteAttachmentDraft = false
            WorkbenchStore.shared.errorMessage = "Could not restore unsent attachments. The original draft file was kept.\n\(error.localizedDescription)"
        }
    }
    ...
    private func persistAttachmentDraft() async -> Bool {
        ...
        guard canWriteAttachmentDraft else {
            WorkbenchStore.shared.errorMessage = "The original attachment draft could not be opened. Your current attachments remain in this window."
            return false
        }
        ...
        do {
            ...
            WorkbenchStore.shared.errorMessage = "Could not save unsent attachments. They remain in this window.\n\(error.localizedDescription)"
            return false
        }
    }
    ...
    private func loadOutbox(sentMessageIDs: Set<UUID>) async {
        do {
            ...
            WorkbenchStore.shared.errorMessage = "Could not restore queued messages. The original queue file was kept.\n\(error.localizedDescription)"
        }
    }
    ...
    func editQueuedPrompt(_ id: UUID, text: String) async throws {
        guard !text.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ||
            ...
            throw LocalWorkbenchError.invalid("Write a message or keep an attachment.")
        }
        guard await updateOutbox({ value in
            guard let index = value.queued.firstIndex(where: { $0.id == id }) else {
                throw LocalWorkbenchError.invalid("This queued message has already been sent or removed.")
            }
            value.queued[index].message.text = text
        }) else { throw LocalWorkbenchError.invalid("The queued message could not be saved. Your original message was kept.") }
    }
    ...
    private func updateOutbox(_ change: @escaping @MainActor (inout ConversationOutbox) throws -> Void) async -> Bool {
        guard canWriteOutbox else {
            WorkbenchStore.shared.errorMessage = "The saved queue needs recovery before it can be changed. Its file was preserved."
            return false
        }
        ...
        let operation = Task { [weak self] in
            ...
            do {
                ...
                outbox = next
                WorkbenchStore.shared.updateThread(chatId) { $0.hasQueuedMessages = !next.queued.isEmpty || next.inFlight != nil }
                return true
            } catch {
                outbox.pauseReason = "The queue could not be saved. Try resuming after checking local storage."
                WorkbenchStore.shared.errorMessage = error.localizedDescription
                return false
            }
        }
    ...
    private func drainQueue() {
        guard !isClosingSession, !isSending, !isLoadingHistory, !isUpdatingQueue, queueDrainTask == nil,
              ...
              WorkbenchStore.shared.thread(chatId).deletedAt == nil else { return }
        queueDrainTask = Task { [weak self] in
            ...
            switchModel(to: WorkbenchModelCatalog.shared.model(prompt.modelID))
            ...
            guard await updateOutbox({ value in
                guard value.queued.first?.id == prompt.id else { throw LocalWorkbenchError.invalid("The next queued message changed.") }
    ...
    private func finishQueuedRun(_ outcome: LocalRunStatus) async {
    ...
    func sendMessage() {
        ...
        guard !isSending, !isLoadingHistory, !sharedMediaDataSource.isImporting, !WorkbenchStore.shared.isRelocating,
                !userInput.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty || !sharedMediaDataSource.isEmpty else { return }
    ...
    func sendPreparedMessage(_ message: MessageData) -> Bool {
        guard WorkbenchValidationMode.permitsInference(
            modelID: pendingModel?.id ?? chatModel.id,
            runtimeEndpoint: backendModel.backend.runtimeEndpoint
        ) else {
            WorkbenchStore.shared.errorMessage = "AWS requests are disabled in this isolated UI test run."
            return false
        }
        guard !isClosingSession, !isSending, !isLoadingHistory, !WorkbenchStore.shared.isRelocating else { return false }
        ...
        guard chatManager.canWriteConversation(chatId) else {
            WorkbenchStore.shared.errorMessage = chatManager.persistenceError
            return false
        }
        do {
            _ = try WorkbenchStore.shared.effectiveSystemPrompt(for: chatId)
            ...
            if BedrockModelID.base(chatModel.id).hasPrefix("luma.") {
                ...
                guard validation.isValid else { throw LocalWorkbenchError.invalid("Choose an output S3 bucket in response settings before generating a video.") }
                guard (message.imageBase64Strings?.count ?? 0) <= 2 else { throw LocalWorkbenchError.invalid("Attach at most two video keyframes: the start image and the end image.") }
            }
            if backendModel.backend.isMantleResponsesModel(chatModel.id) &&
                ...
                throw LocalWorkbenchError.invalid("This model currently accepts text only in this app. Choose a vision/document model or remove the attachments.")
            }
            if message.documentBase64Strings?.isEmpty == false && !backendModel.backend.isDocumentChatSupported(chatModel.id) {
                throw LocalWorkbenchError.invalid("This model does not accept document attachments. Choose a document-capable model.")
            }
            if message.imageBase64Strings?.isEmpty == false &&
                ...
                throw LocalWorkbenchError.invalid("This model does not accept images. Choose a vision model to send these attachments.")
            }
        } catch {
            WorkbenchStore.shared.errorMessage = error.localizedDescription
            return false
    ...
    private func validateDemoInputs(_ message: MessageData) throws {
        ...
        if store.thread(chatId).demoID == "project-review" {
            guard store.preferences.enabledTools.contains(.readFile),
                ...
                throw LocalWorkbenchError.invalid("Enable Read files and List files in Settings → Tools & MCP before running this demo.")
            }
        }
        guard messages.isEmpty else { return }
        switch store.thread(chatId).demoID {
        case "document" where message.documentBase64Strings?.isEmpty != false:
            throw LocalWorkbenchError.invalid("Attach a document before running this demo. Your prompt is ready in the message bar.")
        case "vision" where message.imageBase64Strings?.isEmpty != false:
            throw LocalWorkbenchError.invalid("Attach an image or screenshot before running this demo.")
        default: break
        }
    }
    ...
    private func validateImageInputs(_ count: Int) throws {
        if let service = StabilityAIImageService.matching(chatModel.id) {
            ...
            guard count >= required else {
                ...
                throw LocalWorkbenchError.invalid("\(service.displayName) needs \(required == 1 ? "an image" : "two images"). \(detail)")
            }
            ...
            if service == .outpaint && [config.outpaintLeft, config.outpaintRight, config.outpaintUp, config.outpaintDown].allSatisfy({ $0 == 0 }) {
                throw LocalWorkbenchError.invalid("Set at least one extension direction in response settings before outpainting.")
            }
            if [.searchReplace, .searchRecolor].contains(service) && config.searchPrompt.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                throw LocalWorkbenchError.invalid("Describe the object to change in response settings before using \(service.displayName).")
            }
            ...
            guard chatModel.id.contains("sd3") else {
                throw LocalWorkbenchError.invalid("This model creates images from text. Choose Text to Image in response settings, or select SD3.5 Large to edit an image.")
            }
            guard count > 0 else { throw LocalWorkbenchError.invalid("Attach a source image before using Image to Image.") }
        }
    }
    ...
    func waitUntilReady() async {
        await initialLoadTask?.value
        await WorkbenchStore.shared.waitForSkills()
    }

    func branchAndSend(prompt: Message, text: String? = nil) async throws {
        guard !isSending, !isLoadingHistory else { throw LocalWorkbenchError.invalid("Wait for this response to finish or stop it first.") }
        ...
        let branch = try await WorkbenchActions.branch(chatModel, messages: prefix,
        ...
        WorkbenchStore.shared.selectThread(branch.chatId)
    ...
    private func sendMessageAsync(_ preparedMessage: MessageData) async {
        ...
        if WorkbenchStore.shared.preferences.automaticTitles {
        ...
        let runID = WorkbenchStore.shared.beginRun(threadID: chatId, modelID: chatModel.id, title: tempInput, automationID: nextAutomationID)
        nextAutomationID = nil
        activeRunID = runID
        var outcome: LocalRunStatus = .completed
        var failure: String?
        ...
        WorkbenchStore.shared.finishRun(runID, status: outcome, error: failure)
        await finishQueuedRun(outcome)
        if outcome == .completed || outcome == .failed {
            await WorkbenchNotifications.shared.post(title: outcome == .completed ? "Response ready" : "Bedrock request failed",
    ...
    private func convertMCPToolsToBedrockFormat(_ tools: [MCPToolInfo]) throws -> AWSBedrockRuntime.BedrockRuntimeClientTypes.ToolConfiguration {
        let bedrockTools: [AWSBedrockRuntime.BedrockRuntimeClientTypes.Tool] = try tools.map { info in
            ...
            guard let schema = try JSONSerialization.jsonObject(with: data) as? [String: Any] else {
                throw LocalWorkbenchError.invalid("The input schema for MCP tool \(info.toolName) must be a JSON object.")
            }
    ...
    private func handleTextLLMWithConverseStream(_ userMessage: MessageData) async throws {
        ...
        if backendModel.backend.isStreamingToolUseSupported(chatModel.id) {
            let localTools = LocalToolManager.specifications(threadID: chatId)
        ...
        let systemPrompt = try WorkbenchStore.shared.effectiveSystemPrompt(for: chatId, availableToolNames: availableToolNames)
    ...
    private func processToolCycles(
        ...
        for cycle in turnCount..<max(1, maxTurns) {
            ...
            let stream = try await backendModel.backend.converseStream(
                ...
                usageHandler: { @Sendable [weak self] usage in
                    Task { @MainActor [weak self] in
                        ...
                        if let runID { WorkbenchStore.shared.recordUsage(usage, runID: runID) }
                    }
                }
            )
            ...
            if calls.isEmpty {
                if WorkbenchStore.shared.preferences.thinkingSummaries && !thinking.isEmpty {
            ...
            for index in storedCalls.indices {
                ...
                if Task.isCancelled {
                    ...
                    if let runID { WorkbenchStore.shared.updateRun(runID) { $0.toolCalls += 1 } }
                }
            ...
            if cycle + 1 >= max(1, maxTurns) {
                throw LocalWorkbenchError.invalid("Stopped after \(maxTurns) tool turns. The results are saved. Send a follow-up to continue or change the limit in Tools & MCP.")
            }
        }
    }
    ...
    private func displayStream(id: UUID, text: String, thinking: String, signature: String) {
        ...
        if let activeRunID, !text.isEmpty || !thinking.isEmpty {
            if WorkbenchStore.shared.state.runs.first(where: { $0.id == activeRunID })?.firstTokenAt == nil {
                WorkbenchStore.shared.updateRun(activeRunID) { $0.firstTokenAt = Date() }
            }
        }
    }
    ...
    private func executeSendableMCPTool(id: String, name: String, input: [String: Any]) async -> SendableToolResult {
        ...
        guard await LocalToolManager.authorize(name: name, input: jsonInput, threadID: chatId) else {
        ...
        if let kind = LocalToolKind(rawValue: name) {
            return await LocalToolManager.execute(kind: kind, input: jsonInput, threadID: chatId, modelID: chatModel.id)
        }
        ...
        return SendableToolResult(status: status,
            text: LocalFileTools.bounded(text, limit: WorkbenchStore.shared.preferences.validToolOutputLimit),
    ...
    private func triggerThinkingSummary(for messageId: UUID, thinking: String) {
        guard WorkbenchStore.shared.preferences.thinkingSummaries, !thinkingCompleted else { return }
    ...
    private func boundedHistory(_ history: [BedrockMessage]) throws -> [BedrockMessage] {
        ...
        let systemSize = try WorkbenchStore.shared.effectiveSystemPrompt(for: chatId).count
        let selection = try ContextBudget.select(sizes, budget: WorkbenchStore.shared.preferences.validContextBudget - systemSize)
        contextNotice = selection.notice
        WorkbenchStore.shared.updateThread(chatId) { $0.contextNotice = selection.notice }
    ...
    private func convertConversationHistoryToBedrockMessages(_ history: ConversationHistory) -> [BedrockMessage] {
        ...
        let hasLocalTools = !LocalToolManager.specifications(threadID: chatId).isEmpty
    ...
    private func handleMantleResponsesModel(_ userMessage: MessageData) async throws {
        ...
        let systemPrompt = try WorkbenchStore.shared.effectiveSystemPrompt(for: chatId, availableToolNames: [])
        ...
        do {
            for _ in 0..<180 {
                ...
                case .completed:
                    ...
                    return
                case .failed:
                    throw LocalWorkbenchError.invalid(job.failureMessage ?? "Video generation failed.")
                case .inProgress:
                    try await Task.sleep(for: .seconds(5))
                }
            }
            throw LocalWorkbenchError.invalid("Video generation is still running. Check the job in Bedrock or the output S3 location.")
        } catch {
    ...
    private func handleTextLLMWithNonStreaming(_ userMessage: MessageData) async throws {
        // Backend normalizes Converse responses into the same event stream. Tool
