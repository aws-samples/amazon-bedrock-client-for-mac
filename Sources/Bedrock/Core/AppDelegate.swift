...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    @objc func newChat(_ sender: Any?) {
        ...
        if let newThread = WorkbenchWindows.newThread { newThread() }
    ...
    @objc func deleteChat(_ sender: Any?) {
        guard WorkbenchWindows.isMainWindowKey,
              WorkbenchStore.shared.destination == .chats,
              let id = WorkbenchStore.shared.selectedThreadID else { return }
        WorkbenchStore.shared.trash(id)
    }
    ...
    private func performUpdateCheck() {
        guard !WorkbenchValidationMode.isOffline else { return }
    ...
    func applicationShouldTerminate(_ sender: NSApplication) -> NSApplication.TerminateReply {
        ...
        Task {
            ...
            if saved { await MCPManager.shared.shutdown() }
            WorkbenchStore.shared.flush()
    ...
    func applicationWillTerminate(_ notification: Notification) {
        ...
        MCPManager.shared.markMCPRunning(false)
        WorkbenchStore.shared.flush()
    ...
    func applicationShouldHandleReopen(_ sender: NSApplication, hasVisibleWindows flag: Bool) -> Bool {
        if !flag { WorkbenchWindows.showMain() }
        return true
    }
