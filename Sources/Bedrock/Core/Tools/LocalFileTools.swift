...
enum LocalPath {
    static func resolve(_ path: String, in root: URL, allowRoot: Bool = true) throws -> URL {
        guard !path.contains("\0"), !path.hasPrefix("~") else { throw LocalOperationError.outsideProject }
        ...
        guard contains(resolved, in: canonicalRoot),
              allowRoot || resolved.path != canonicalRoot.path else { throw LocalOperationError.outsideProject }
        return resolved
    }
    ...
    static func canonicalize(_ url: URL) throws -> URL {
        ...
        while !pending.isEmpty {
            ...
            if let link = try? FileManager.default.destinationOfSymbolicLink(atPath: candidate.path) {
                followedLinks += 1
                guard followedLinks <= 40 else { throw LocalOperationError.invalid("The path contains a symbolic-link loop.") }
    ...
    static func validatedWebURL(_ value: String, allowedDomains: String) throws -> URL {
        guard let url = URL(string: value), let scheme = url.scheme?.lowercased(),
            ...
            throw LocalOperationError.invalid("Use an HTTP or HTTPS URL without embedded credentials.")
        }
        ...
        if !domains.isEmpty && !domains.contains(where: { host == $0 || host.hasSuffix("." + $0) }) {
            throw LocalOperationError.invalid("“\(host)” is not in the allowed web domains.")
        }
        return url
    }
}
...
enum LocalFileTools {
    ...
    static func list(root: URL, directory: String = ".", limit: Int = 1_000,
        ...
        guard FileManager.default.fileExists(atPath: target.path, isDirectory: &isDirectory), isDirectory.boolValue,
            ...
            throw LocalOperationError.unavailable("This folder is unavailable or macOS has not granted access to it.")
        }
        guard let enumerator = FileManager.default.enumerator(at: target, includingPropertiesForKeys: Array(keys),
            ...
            throw LocalOperationError.unavailable("This folder could not be opened.")
        }
    ...
    static func read(root: URL, path: String, maximumBytes: Int = 1_048_576, startLine: Int = 1, lineCount: Int = 300) throws -> String {
        ...
        guard let text = String(data: data, encoding: .utf8), !text.contains("\0") else {
            throw LocalOperationError.invalid("This file is not UTF-8 text. Use the native preview to open it.")
        }
    ...
    static func readData(root: URL, path: String, limit: Int) throws -> Data {
        try Task.checkCancellation()
        guard limit > 0 else { throw LocalOperationError.invalid("The file size limit must be positive.") }
        let url = try LocalPath.resolve(path, in: root, allowRoot: false)
        let values = try url.resourceValues(forKeys: [.isRegularFileKey, .fileSizeKey])
        guard values.isRegularFile == true else { throw LocalOperationError.invalid("Select a regular file.") }
    ...
    static func search(root: URL, query: String, maximumResults: Int = 100) throws -> String {
        guard !query.isEmpty else { throw LocalOperationError.invalid("Enter text to search for.") }
        var results: [String] = []
    ...
    static func write(root: URL, path: String, content: String, maximumBytes: Int = 1_048_576) throws -> String {
        try Task.checkCancellation()
        guard content.utf8.count <= maximumBytes else { throw LocalOperationError.tooLarge(maximumBytes) }
        let url = try LocalPath.resolve(path, in: root, allowRoot: false)
