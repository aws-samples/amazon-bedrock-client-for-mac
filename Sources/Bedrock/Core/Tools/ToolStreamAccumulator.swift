...
struct ToolStreamAccumulator: Sendable {
    ...
    mutating func begin(index: Int, id: String, name: String) throws {
        guard calls[index] == nil, !calls.values.contains(where: { $0.id == id }), !id.isEmpty, !name.isEmpty else {
            throw LocalOperationError.invalid("The model returned a duplicate or incomplete tool call.")
        }
    ...
    mutating func append(index: Int, json: String) throws {
        guard var call = calls[index], !call.completed else {
            throw LocalOperationError.invalid("The model returned tool input without a matching start.")
        }
        guard call.inputJSON.utf8.count + json.utf8.count <= 1_048_576 else { throw LocalOperationError.tooLarge(1_048_576) }
        call.inputJSON += json
        calls[index] = call
    }
    mutating func complete(index: Int) throws {
        ...
        guard let data = call.inputJSON.data(using: .utf8),
            ...
            throw LocalOperationError.invalid("The model returned invalid JSON input for \(call.name).")
        }
        call.completed = true
        calls[index] = call
    }
    func finish() throws -> [StreamedToolCall] {
        guard calls.values.allSatisfy(\.completed) else {
            throw LocalOperationError.invalid("The response ended before all tool inputs arrived. Retry the response.")
        }
