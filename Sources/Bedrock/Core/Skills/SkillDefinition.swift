...
struct SkillDefinition: Identifiable, Equatable, Sendable {
    var id: String
    var name: String
    ...
    func unavailableReason(environment: [String: String] = ProcessInfo.processInfo.environment,
                           executableExists: (String) -> Bool = SkillDefinition.executableExists) -> String? {
    ...
    static func parse(_ raw: String, id: String, url: URL) throws -> SkillDefinition {
        guard raw.utf8.count <= 1_048_576 else { throw LocalOperationError.tooLarge(1_048_576) }
        ...
        if lines.first?.trimmingCharacters(in: .whitespaces) == "---" {
            guard let end = lines.dropFirst().firstIndex(where: { $0.trimmingCharacters(in: .whitespaces) == "---" }) else {
                throw LocalOperationError.invalid("The skill frontmatter is missing its closing --- line.")
            }
            fields = try parseFrontmatter(Array(lines[1..<end]))
            body = lines.dropFirst(end + 1).joined(separator: "\n")
        }
        body = body.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !body.isEmpty else { throw LocalOperationError.invalid("Add instructions to this skill.") }
        let fallbackName = id.replacingOccurrences(of: "-", with: " ").capitalized
        let name = scalar(fields["name"] ?? fallbackName)
        guard !name.isEmpty else { throw LocalOperationError.invalid("A skill needs a name.") }
    ...
    private static func parseFrontmatter(_ lines: [String]) throws -> [String: String] {
        ...
        while index < lines.count {
            ...
            guard let colon = trimmed.firstIndex(of: ":") else {
                throw LocalOperationError.invalid("Invalid skill frontmatter near “\(trimmed.prefix(60))”.")
            }
    ...
    static func context(for selectedIDs: [String], skills: [SkillDefinition], enabled: [String: Bool], limit: Int = 32_000,
        ...
        for id in selectedIDs {
            ...
            if let reason { throw LocalOperationError.unavailable("\(skill.name): \(reason)") }
            sections.append("<skill name=\"\(skill.name.replacingOccurrences(of: "\"", with: "'"))\">\n\(skill.instructions)\n</skill>")
        }
        let text = sections.joined(separator: "\n\n")
        guard text.count <= limit else {
            throw LocalOperationError.invalid("The selected skills exceed the \(limit.formatted()) character budget. Select fewer skills or shorten their instructions.")
        }
        return text
    }
