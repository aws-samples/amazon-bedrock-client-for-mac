...
enum ContextBudget {
    ...
    static func select(_ messages: [ContextMessageSize], budget: Int) throws -> ContextSelection {
        ...
        guard selectedStart < messages.count else {
            throw LocalOperationError.invalid("The latest turn exceeds the context budget. Increase the budget in Models & context, shorten the prompt, or start a new thread.")
        }
