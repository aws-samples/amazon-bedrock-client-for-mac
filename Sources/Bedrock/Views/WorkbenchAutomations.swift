...
struct WorkbenchAutomationsView: View {
    @ObservedObject private var store = WorkbenchStore.shared
    @ObservedObject private var runner = WorkbenchAutomationRunner.shared
    @State private var editing: LocalAutomation?
    @State private var deleting: LocalAutomation?
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                WorkbenchPageHeader(title: "A little help, on schedule", subtitle: "Repeat useful prompts while Bedrock is open on this Mac. Missed runs are skipped after sleep; completed work stays in your threads.")
                HStack {
                    Toggle("Schedules enabled", isOn: Binding(get: { store.preferences.automationsEnabled }, set: { store.preferences.automationsEnabled = $0 }))
                        .toggleStyle(WorkbenchSwitchStyle()).fixedSize()
                    Spacer()
                    Button("New automation", systemImage: "plus") {
                        editing = LocalAutomation(name: "", prompt: "", modelID: WorkbenchModelCatalog.shared.defaultModel?.id ?? "")
                    }
                }
                if store.state.automations.isEmpty {
                    WorkbenchEmptyState(symbol: "clock.arrow.2.circlepath", title: "Put a useful prompt on repeat", detail: "Create a writing exercise or a repeatable Bedrock demo. New automations start paused.")
                }
                ForEach(store.state.automations) { automation in
                    VStack(alignment: .leading, spacing: 14) {
                        HStack(alignment: .top, spacing: 13) {
                            Image(systemName: "clock").font(.system(size: 19, weight: .light)).foregroundStyle(WorkbenchStyle.accent).padding(.top, 3)
                            ...
                            Toggle("Enabled", isOn: Binding(get: { automation.enabled }, set: { runner.setEnabled(automation.id, enabled: $0) }))
                                .toggleStyle(WorkbenchSwitchStyle(showsLabel: false)).labelsHidden().help("Enable \(automation.name)")
                        }
                        HStack(spacing: 14) {
                    ...
                    .padding(20).background(WorkbenchStyle.surface.opacity(0.6), in: RoundedRectangle(cornerRadius: 13))
                    .overlay(RoundedRectangle(cornerRadius: 13).stroke(WorkbenchStyle.border, lineWidth: 0.5))
                }
            }
            .padding(32).frame(maxWidth: 1050).frame(maxWidth: .infinity)
        }
        .sheet(item: $editing) { WorkbenchAutomationEditor(automation: $0) }
...
struct WorkbenchAutomationEditor: View {
    @State var automation: LocalAutomation
        ...
        VStack(alignment: .leading, spacing: 18) {
            ...
            Form {
                Section {
                    ...
                    WorkbenchMenuField(title: "Model", selection: $automation.modelID,
                    ...
                    TextEditor(text: $automation.prompt).font(.system(size: 13)).frame(height: 120).border(WorkbenchStyle.border)
                }
                Section("Schedule") {
                    WorkbenchMenuField(title: "Repeat", selection: $automation.cadence,
            ...
            HStack {
                ...
                Button("Save automation") {
                    do { try WorkbenchAutomationRunner.shared.save(automation); dismiss() }
