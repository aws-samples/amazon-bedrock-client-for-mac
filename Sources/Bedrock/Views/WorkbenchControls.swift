...
struct WorkbenchToolbarIcon: View {
    let symbol: String
    ...
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .foregroundStyle(configuration.role == .destructive ? Color.red :
                                prominent ? WorkbenchStyle.canvas : Color.primary)
            .padding(.horizontal, size == .small || size == .mini ? 9 : 12)
            .frame(minHeight: size == .small || size == .mini ? 26 : 32)
            .background {
                RoundedRectangle(cornerRadius: 8)
                    .fill(prominent ? WorkbenchStyle.accent : Color.primary.opacity(configuration.isPressed ? 0.10 : hovered ? 0.065 : 0.035))
            }
            .overlay(RoundedRectangle(cornerRadius: 8).strokeBorder(WorkbenchStyle.border))
            .contentShape(RoundedRectangle(cornerRadius: 8))
            .opacity(enabled ? 1 : 0.38)
            .scaleEffect(configuration.isPressed && !reduceMotion ? 0.98 : 1)
            .onHover { hovered = $0 }
            .animation(reduceMotion ? nil : WorkbenchMotion.feedback, value: configuration.isPressed)
            .animation(reduceMotion ? nil : WorkbenchMotion.feedback, value: hovered)
    }
}

struct WorkbenchSwitchStyle: ToggleStyle {
    var showsLabel = true
    @Environment(\.isEnabled) private var enabled
    @Environment(\.accessibilityReduceMotion) private var reduceMotion
    func makeBody(configuration: Configuration) -> some View {
        Button { configuration.isOn.toggle() } label: {
            HStack(spacing: 12) {
                if showsLabel {
                    configuration.label.font(WorkbenchStyle.body)
                    Spacer(minLength: 8)
                }
                Capsule()
                    .fill(configuration.isOn ? WorkbenchStyle.selection : Color.primary.opacity(0.16))
                    .overlay {
        ...
        .animation(reduceMotion ? nil : WorkbenchMotion.standard, value: configuration.isOn)
    }
}

struct WorkbenchSegmentedControl<Value: Hashable>: View {
    var title: String
    ...
    var body: some View {
        HStack(spacing: 3) {
            ForEach(options.indices, id: \.self) { index in
                ...
                Button { selection = option.value } label: {
                    Text(option.title).font(WorkbenchStyle.caption)
                        .frame(maxWidth: .infinity).padding(.vertical, 7)
                        .foregroundStyle(selection == option.value ? .primary : .secondary)
                        .background(selection == option.value ? WorkbenchStyle.field : .clear,
                                    in: RoundedRectangle(cornerRadius: 6))
                        .overlay(RoundedRectangle(cornerRadius: 6).strokeBorder(selection == option.value ? WorkbenchStyle.border : .clear))
        ...
        .animation(reduceMotion ? nil : WorkbenchMotion.feedback, value: selection)
    }
}
...
struct WorkbenchMenuField<Value: Hashable>: View {
    let title: String
    @Binding var selection: Value
    ...
    var body: some View {
        Button { presented.toggle() } label: {
            HStack(spacing: 8) {
                Text(options.first(where: { $0.value == selection })?.title ?? "Select…")
                    .font(WorkbenchStyle.body).lineLimit(1)
                Spacer(minLength: 4)
            ...
            .overlay(RoundedRectangle(cornerRadius: 8).strokeBorder(presented ? WorkbenchStyle.selection.opacity(0.65) : WorkbenchStyle.border))
            .contentShape(RoundedRectangle(cornerRadius: 8))
        }
        ...
        .popover(isPresented: $presented, arrowEdge: .bottom) {
            WorkbenchMenuOptions(title: title, selection: $selection, options: options) { presented = false }
        }
    }
}

private struct WorkbenchMenuOptions<Value: Hashable>: View {
    let title: String
    @Binding var selection: Value
    let options: [(value: Value, title: String)]
    var close: () -> Void
    ...
    var body: some View {
        VStack(spacing: 6) {
            if options.count > 8 {
                ...
                .font(WorkbenchStyle.body).padding(9)
                .background(WorkbenchStyle.field, in: RoundedRectangle(cornerRadius: 7)).padding(6)
            }
                    ...
                    LazyVStack(spacing: 2) {
                        ForEach(visible, id: \.self) { index in
                            Button { choose(index) } label: {
                                HStack(spacing: 9) {
                                    Text(options[index].title).font(WorkbenchStyle.body).lineLimit(2)
                                    Spacer(minLength: 8)
                        ...
                        if visible.isEmpty { Text("No matches").font(WorkbenchStyle.caption).foregroundStyle(.secondary).padding(16) }
                    }.padding(6)
                }
...
struct WorkbenchActionMenu<Label: View, Content: View>: View {
    ...
    var body: some View {
        Button { presented.toggle() } label: {
            label().font(WorkbenchStyle.body)
        ...
        .popover(isPresented: $presented, arrowEdge: .bottom) {
            ScrollView {
                VStack(alignment: .leading, spacing: 2) { content() }
                    .buttonStyle(WorkbenchMenuActionStyle(close: { presented = false }))
                    .padding(6)
            }
...
extension WorkbenchActionMenu where Label == Text {
...
private struct WorkbenchMenuActionStyle: PrimitiveButtonStyle {
    var close: () -> Void
    func makeBody(configuration: Configuration) -> some View {
        Button {
            close()
        ...
        }.buttonStyle(WorkbenchMenuRowStyle())
    }
}

private struct WorkbenchMenuRowStyle: ButtonStyle {
    @State private var hovered = false
    @Environment(\.isEnabled) private var enabled
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
