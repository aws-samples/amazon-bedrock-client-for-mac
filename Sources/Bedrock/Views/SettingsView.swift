...
struct SettingsView: View {
    ...
    @StateObject private var backend = BackendModel()
    @State private var pane: WorkbenchSettingsPane? = .general
    ...
    private var rows: [WorkbenchSetting] { WorkbenchSetting.all.filter { $0.pane == (pane ?? .general) } }
    ...
    var body: some View {
        HStack(spacing: 0) {
            VStack(spacing: 12) {
                WorkbenchSearchField(placeholder: "Search settings", text: $query)
                    .padding(.horizontal, 12).padding(.top, 16)
                if query.isEmpty {
                    List(selection: $pane) {
                        ForEach(WorkbenchSettingsPane.allCases) { item in
                            Label {
                                Text(item.title)
                            } icon: {
                                Image(systemName: item.symbol).foregroundStyle(.primary)
                            }.font(WorkbenchStyle.body).padding(.vertical, 4)
                    ...
                    List {
                        ForEach(WorkbenchSetting.all.filter { $0.matches(query) }) { row in
                ...
                Text("Bedrock for Mac").font(WorkbenchStyle.detail)
                    .foregroundStyle(.secondary).padding(.bottom, 18)
            }
            .frame(width: 190)
            Color.clear.frame(width: 1)
            VStack(alignment: .leading, spacing: 0) {
                WorkbenchPageHeader(title: (pane ?? .general).title, subtitle: (pane ?? .general).subtitle)
                    .padding(.horizontal, 24).padding(.top, 24).padding(.bottom, 2)
                if pane == .skills {
                    WorkbenchSkillsLibrary()
                } else {
        ...
        .background {
            WorkbenchSplitSurface(sidebarWidth: 190)
        }
        .background(WorkbenchWindowChrome(hidesTitle: false))
        ...
        .toggleStyle(WorkbenchSwitchStyle())
        .sheet(isPresented: $showAddServer) { ServerFormView(isPresented: $showAddServer) }
        .sheet(item: $selectedTool) { WorkbenchMCPToolDefinition(tool: $0) }
        .sheet(isPresented: $showHistory) { WorkbenchHistorySettingsView() }
        .onAppear {
    ...
    private func settingRow(_ row: WorkbenchSetting) -> some View {
        VStack(alignment: .leading, spacing: 6) {
            control(row)
            Text(row.detail).font(WorkbenchStyle.detail).foregroundStyle(.secondary).fixedSize(horizontal: false, vertical: true)
        }
        .padding(.vertical, 7)
    ...
    @ViewBuilder private func control(_ row: WorkbenchSetting) -> some View {
        switch row.id {
        ...
        case "appearance":
            WorkbenchSegmentedControl(title: row.title, selection: $settings.appearance,
        ...
        case "region":
            settingsField(row.title) {
                WorkbenchMenuField(title: row.title, selection: $settings.selectedRegion,
                                   options: AWSRegion.allCases.map { ($0, "\($0.name) · \($0.rawValue)") })
            }
        case "profile":
            settingsField(row.title) {
                HStack(spacing: 10) {
                    WorkbenchMenuField(title: row.title, selection: $settings.selectedProfile,
            ...
            settingsField(row.title) {
                HStack(spacing: 10) {
                    SecureField("Optional API key", text: $apiKey)
                        ...
                        .background(WorkbenchStyle.field, in: RoundedRectangle(cornerRadius: 7))
                        .overlay(RoundedRectangle(cornerRadius: 7).strokeBorder(WorkbenchStyle.border))
        ...
        case "defaultModel":
            settingsField(row.title) {
                HStack(spacing: 10) {
                    ModelSelectorDropdown(organizedChatModels: catalog.organized, menuSelection: defaultModelSelection) { value in
                        if case .chat(let model) = value { settings.defaultModelId = model.id }
                    }
                    Spacer(minLength: 8)
                    if let model = catalog.defaultModel {
                        InferenceConfigDropdown(currentModelId: .constant(model.id), backend: backend.backend)
                    }
                }
                if let model = catalog.defaultModel {
                    Text(model.id)
        ...
        case "systemPrompt": SystemPromptSection()
        ...
        case "toolProfile":
            settingsField(row.title) {
                WorkbenchMenuField(title: row.title, selection: Binding(get: { store.preferences.toolProfile }, set: { profile in
                    store.preferences.toolProfile = profile
                    store.preferences.disabledTools = []
                }), options: LocalToolProfile.allCases.map { ($0, $0.title) })
            }
        case "approval":
            settingsField(row.title) {
                WorkbenchMenuField(title: row.title, selection: pref(\.approvalMode),
                                   options: LocalApprovalMode.allCases.map { ($0, $0.title) })
            }
        case "fileAccess":
        ...
        case "send":
            WorkbenchSegmentedControl(title: row.title, selection: pref(\.sendWithCommandReturn),
        ...
        case "notifications":
            Toggle(row.title, isOn: Binding(get: { store.preferences.notificationsEnabled }, set: { value in
                ...
                Task {
                    let granted = await WorkbenchNotifications.shared.enable()
        ...
        case "notificationTest": Button(row.title) { Task { if await WorkbenchNotifications.shared.enable() { await WorkbenchNotifications.shared.post(title: "Bedrock is ready", body: "Your run notifications are working.", test: true) } } }
        case "dataDirectory":
        ...
        case "diagnostics": Button(row.title, action: WorkbenchActions.exportDiagnostics)
        case "about":
        ...
        default:
            if let tool = LocalToolKind(rawValue: row.id) {
    ...
    private var mcpControls: some View {
        VStack(alignment: .leading, spacing: 13) {
            ...
            ForEach(mcp.servers) { server in ServerRow(serverName: server.name) }
            if mcp.mcpEnabled && !mcp.toolInfos.isEmpty {
                WorkbenchActionMenu("Inspect connected tools") {
            ...
            HStack {
                ...
                Button("Import…") { WorkbenchMCPConfiguration.importFile() }
                Button("Export…") { WorkbenchMCPConfiguration.exportFile() }
    ...
    private func pref<Value>(_ keyPath: WritableKeyPath<WorkbenchPreferences, Value>) -> Binding<Value> {
    ...
    private func open(_ id: String) {
        ...
        guard let row = WorkbenchSetting.all.first(where: { $0.id == id }) else { return }
        pane = row.pane
    ...
    private func updateHotkey() {
        ...
        HotkeyManager.shared.updateHotkey(modifiers: modifiers, keyCode: keyCode)
    }
...
struct WorkbenchMCPToolDefinition: View {
        ...
        VStack(alignment: .leading, spacing: 16) {
            HStack(alignment: .top, spacing: 12) {
                VStack(alignment: .leading, spacing: 5) {
                    ...
                    Text(tool.serverName).font(WorkbenchStyle.caption).foregroundStyle(.secondary)
                }
                Spacer()
                Button("Done") { dismiss() }.keyboardShortcut(.cancelAction)
            }
            ScrollView {
                Text(tool.description).font(WorkbenchStyle.body).textSelection(.enabled)
                    .frame(maxWidth: .infinity, alignment: .leading)
            }.frame(maxHeight: 100)
            HStack {
                Text("Input schema").font(WorkbenchStyle.label)
                Spacer()
                Button("Copy schema") {
                    NSPasteboard.general.clearContents()
