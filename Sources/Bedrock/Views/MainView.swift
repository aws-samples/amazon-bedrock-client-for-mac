...
struct MainView: View {
    ...
    @StateObject private var backendModel = BackendModel()
    ...
    @StateObject private var newThreadDraft = WorkbenchComposerDraft.welcome
    private var newThreadMedia: SharedMediaDataSource { newThreadDraft.media }
    ...
    @State private var navigationHistory = WorkbenchNavigationHistory()
    @Environment(\.accessibilityReduceMotion) private var reduceMotion

    private var location: WorkbenchNavigationLocation {
    ...
    private var windowContent: some View {
        GeometryReader { window in
            HStack(spacing: 0) {
                Group {
                    if sidebarVisible {
                        WorkbenchSidebar(onNew: beginNewThread)
    ...
    private var styledContent: some View {
        ...
        .background(WorkbenchWindowChrome())
        ...
        .overlay {
            if store.showCommandPalette {
                ZStack {
                    ...
                    WorkbenchCommandPalette(onNewThread: beginNewThread, onDemo: useDemo, onDismiss: closeSearch)
                        .clipShape(RoundedRectangle(cornerRadius: 14))
                        .overlay(RoundedRectangle(cornerRadius: 14).strokeBorder(WorkbenchStyle.border))
    ...
    private var navigationContent: some View {
        ...
        .onAppear {
            ...
            WorkbenchWindows.newThread = beginNewThread
        ...
        .task {
            ...
            WorkbenchAutomationRunner.shared.start(backend: backendModel)
        }
    ...
    private var presentedContent: some View {
        ...
        .sheet(isPresented: $showApproval, onDismiss: {
            ...
            if let approval { WorkbenchToolApprovalView(request: approval) }
        }
        .sheet(item: $demoToConfigure) { demo in
            WorkbenchDemoVariablesSheet(demo: demo) { prompt in
                demoToConfigure = nil
                prepareDemo(demo, prompt: prompt)
            }
        }
        .sheet(isPresented: $showComparison) {
            WorkbenchComparisonSheet(models: catalog.models.filter { compatible($0, with: .text) }, backend: backendModel)
        }
    ...
    @ViewBuilder private var detail: some View {
        ...
        case .demos: WorkbenchDemoLibrary(onUse: useDemo)
        case .automations: WorkbenchAutomationsView()
        case .activity: WorkbenchActivityView()
        }
    }
    ...
    @ToolbarContentBuilder private var toolbar: some ToolbarContent {
        if store.destination != .chats {
            ToolbarItem(placement: .principal) {
                Text(store.destination.title).font(WorkbenchStyle.label)
            }
        }
    ...
    private var toolbarActions: some View {
        Group {
            if let selectedChat, store.destination == .chats {
                WorkbenchActionMenu(horizontalPadding: 0) { WorkbenchThreadMenu(chat: selectedChat) } label: {
                    WorkbenchToolbarIcon("ellipsis")
            ...
            Button { store.showCommandPalette.toggle() } label: {
                WorkbenchToolbarIcon("magnifyingglass")
            }
    ...
    private var navigationButtons: some View {
        ...
        Group {
            Button(action: toggleSidebar) { WorkbenchToolbarIcon("sidebar.left", weight: .medium) }
                .buttonStyle(LiquidGlassToolbarButtonStyle())
                .help("Toggle sidebar (⌘B)")
                .accessibilityLabel(sidebarVisible ? "Hide sidebar" : "Show sidebar")
                .accessibilityIdentifier("toolbar.toggleSidebar")
            Button(action: goBack) { WorkbenchToolbarIcon("arrow.left") }
                .buttonStyle(LiquidGlassToolbarButtonStyle())
                .disabled(!canGoBack)
    ...
    private func canOpen(_ location: WorkbenchNavigationLocation) -> Bool {
...
private extension MainView {
    var welcome: some View {
        VStack(spacing: 0) {
            ...
            VStack(spacing: 24) {
                ...
                MessageBarView(chatID: "new-thread", userInput: $newDraftText, sharedMediaDataSource: newThreadMedia,
    ...
    func beginNewThread() {
        ...
        let chat = WorkbenchActions.newThread(model: model)
    ...
    func sendNewThread() async {
        ...
        let chat = WorkbenchActions.newThread(model: model, draft: newDraftText, skillIDs: metadata.skillIDs)
    ...
    func sendQuickAccess() {
        ...
        let chat = WorkbenchActions.newThread(model: model, draft: coordinator.quickAccessMessage ?? "")
        let session = ChatSessionPool.shared.session(chatID: chat.chatId, backend: backendModel)
    ...
    func copyMedia(_ source: SharedMediaDataSource, to destination: SharedMediaDataSource) {
    ...
    func prepareDemo(_ demo: DemoPreset, prompt: String) {
        ...
        let chat = WorkbenchActions.newThread(model: model, draft: prompt, skillIDs: demo.skillIDs, systemPrompt: demo.systemPrompt)
