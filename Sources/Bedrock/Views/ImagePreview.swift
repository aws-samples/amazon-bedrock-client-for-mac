...
struct ImagePreviewModal: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            ZStack {
                ...
                if let image {
                    ...
                    WorkbenchEmptyState(symbol: "photo", title: "Image could not be opened", detail: loadError)
                } else {
        ...
        .task {
            do {
                let prepared = try await WorkbenchImageIO.shared.load(source)
    ...
    private var header: some View {
        HStack(spacing: 8) {
            ...
            Text(filename).font(WorkbenchStyle.label).lineLimit(1).truncationMode(.middle)
            Spacer(minLength: 12)
    ...
    private func setZoom(_ value: CGFloat) {
        withAnimation(reduceMotion ? nil : WorkbenchMotion.feedback) {
    ...
    private func copy() {
        ...
        actionTask = Task {
            ...
            do {
                let bytes = try await WorkbenchImageIO.shared.export(image, as: .png)
                try Task.checkCancellation()
                NSPasteboard.general.clearContents()
                guard NSPasteboard.general.setData(bytes, forType: .png) else {
                    throw LocalWorkbenchError.invalid("The image could not be copied to the clipboard.")
                }
                copied = true
            } catch is CancellationError {
