...
struct ModelSelectorDropdown: View {
    ...
    var body: some View {
        Button { isShowingPopover.toggle() } label: {
            ...
            .padding(.horizontal, 8).frame(height: WorkbenchStyle.controlSize)
        ...
        .popover(isPresented: $isShowingPopover, arrowEdge: .top) {
            ModelSelectorPopoverContent(models: organizedChatModels.values.flatMap { $0 }, selectedID: selectedModel?.id) { id in
                let model = organizedChatModels.values.lazy.flatMap { $0 }.first { $0.id == id } ?? WorkbenchModelCatalog.shared.model(id)
...
private struct ModelSelectorPopoverContent: View {
    ...
    var body: some View {
        ...
        VStack(spacing: 0) {
            ...
            Divider()
            if rows.isEmpty {
                WorkbenchEmptyState(symbol: "magnifyingglass", title: "No models found", detail: "Try a different search term.")
            } else {
    ...
    private func modelRow(_ row: BedrockModelChoice) -> some View {
                ...
                HStack(spacing: 8) {
                    ...
                    VStack(alignment: .leading, spacing: 3) {
                        HStack(spacing: 6) {
                            ...
                            if selected { Image(systemName: "checkmark").font(.system(size: 10, weight: .bold)).foregroundStyle(WorkbenchStyle.accent) }
                        }
        ...
        .background(selected ? WorkbenchStyle.accent.opacity(colorScheme == .dark ? 0.18 : 0.09) :
                    Color.primary.opacity(hoveredID == row.id || focusedID == row.id ? 0.06 : 0), in: RoundedRectangle(cornerRadius: 6))
        .overlay(RoundedRectangle(cornerRadius: 6).stroke(selected ? WorkbenchStyle.accent.opacity(0.25) : .clear, lineWidth: 0.5))
