...
class ConversationStore: ObservableObject {
    ...
    private var logger = Logger(label: "ConversationStore")
    ...
    private func migrateHistoryForChat(chatId: String, modelId: String, force: Bool = false) {
        ...
        let prompt = PreferencesStore.shared.systemPrompt.trimmingCharacters(in: .whitespacesAndNewlines)
    ...
    func changeModel(for chatID: String, to model: ChatModel) -> ChatModel? {
        ...
        do {
            guard let entity = try coreDataStack.viewContext.fetch(request).first else {
                throw LocalOperationError.unavailable("This conversation could not be found in local storage.")
            }
    ...
    func setIsLoading(_ isLoading: Bool, for chatId: String) {
        // ConversationStore is main-actor isolated. Deferring this mutation leaves
    ...
    func conversationSnapshot(for chatID: String) async throws -> ConversationHistory {
        ...
        guard let chat = getChatModel(for: chatID) else {
            throw LocalOperationError.unavailable("This conversation is no longer available.")
        }
        let legacy = getMessageFileURL(chatId: chatID)
        let worker = Task.detached(priority: .userInitiated) {
            try ConversationFileStore.read(unifiedURL: unified, legacyURL: legacy, chatID: chatID)
        ...
        guard unified == getConversationHistoryFileURL(chatId: chatID), getChatModel(for: chatID) != nil else {
            throw LocalOperationError.unavailable("The conversation location changed. Please try again.")
        }
    ...
    private func createNewConversationHistory(for chatId: String) -> ConversationHistory {
        ...
        let systemPrompt = PreferencesStore.shared.systemPrompt.trimmingCharacters(in: .whitespacesAndNewlines)
    ...
    private func deleteAllFiles(for chatId: String) {
        ...
        if let outboxURL = try? ConversationOutboxFile.url(threadID: chatId, directory: AppStore.shared.directory) {
            files.append(outboxURL)
        }
        if let draftURL = try? ConversationAttachmentDraftFile.url(threadID: chatId, directory: AppStore.shared.directory) {
