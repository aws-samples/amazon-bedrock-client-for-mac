...
final class VideoGenerationService: @unchecked Sendable {
    ...
    func startLumaVideoGeneration(modelID: String, request: Data, s3OutputUri: String) async throws -> String {
        let validation = NovaReelService.validateS3Uri(s3OutputUri)
        guard validation.isValid else { throw VideoGenerationError.invalidS3Uri(validation.message ?? "Choose an output S3 bucket.") }
        ...
        let input = StartAsyncInvokeInput(
            ...
            modelInput: try Self.convertToDocument(body),
            outputDataConfig: .s3outputdataconfig(.init(s3Uri: s3OutputUri))
        )
        let response = try await bedrockRuntimeClient.startAsyncInvoke(input: input)
        guard let arn = response.invocationArn else { throw VideoGenerationError.noInvocationArn }
        return arn
    }
    ...
    /// Get local video cache directory (uses same base as ImageStore)
    @MainActor
    static var videoCacheDirectory: URL {
        let baseDir = URL(fileURLWithPath: PreferencesStore.shared.defaultDirectory)
