...
class ImageGenerationService {
    ...
    init(bedrockRuntimeClient: BedrockRuntimeClient, region: String) {
        self.bedrockRuntimeClient = bedrockRuntimeClient
        self.region = region
    }

    private func invocationID(_ modelID: String) -> String {
        BedrockCapabilityRegistry.shared.invocationID(modelID, region: region)
    }
    ...
    private func invokeTitanImage(modelId: String, prompt: String) async throws -> Data {
        // Use saved config from PreferencesStore
        let config = await MainActor.run { PreferencesStore.shared.titanImageConfig }
    ...
    func invokeNovaCanvas(request: NovaCanvasRequest) async throws -> Data {
        ...
        let invokeRequest = InvokeModelInput(
            body: encodedParams,
            contentType: "application/json",
            modelId: invocationID(NovaCanvasService.modelId)
        )
    ...
    private func invokeStabilityAI(modelId: String, prompt: String) async throws -> Data {
        // Use saved config from PreferencesStore
        let config = await MainActor.run { PreferencesStore.shared.stabilityAIConfig }
    ...
    func invokeStabilityAIService(
        ...
        let config = await MainActor.run { PreferencesStore.shared.stabilityAIServicesConfig }
        ...
        let request = InvokeModelInput(
            body: jsonData,
            contentType: "application/json",
            modelId: invocationID(modelID ?? service.modelId)
        )
