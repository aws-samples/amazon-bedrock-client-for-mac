...
class GlobalHotkeyService: ObservableObject {
    ...
    private let logger = Logger(label: "GlobalHotkeyService")
    private let settingManager = PreferencesStore.shared
    
    // Hotkey settings from PreferencesStore
    ...
    private func setupEventHandler() {
        ...
        let callback: EventHandlerProcPtr = { (nextHandler, theEvent, userData) -> OSStatus in
            ...
            let manager = Unmanaged<GlobalHotkeyService>.fromOpaque(userData).takeUnretainedValue()
    ...
    private func handleHotkeyPressed() {
        ...
        DispatchQueue.main.async {
            QuickAccessWindowController.shared.toggleWindow()
        }
    }
