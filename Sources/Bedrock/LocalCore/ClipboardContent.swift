...
enum ClipboardHTMLParser {
    ...
    static func parse(_ html: String, includeText: Bool = true) throws -> ClipboardHTMLContent {
        guard html.utf8.count <= maximumHTMLBytes else { throw LocalWorkbenchError.tooLarge(maximumHTMLBytes) }
        ...
        func appendText(_ range: Range<Int>) throws {
            ...
            guard text.count + range.count <= maximumTextBytes else { throw LocalWorkbenchError.tooLarge(maximumTextBytes) }
