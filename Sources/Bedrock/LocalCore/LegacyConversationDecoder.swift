...
enum LegacyConversationDecoder {
    static func decode(_ data: Data) throws -> [MessageData] {
        ...
        guard let records = try JSONSerialization.jsonObject(with: data) as? [[String: Any]] else {
            throw LocalWorkbenchError.invalid("The legacy conversation is not a message array.")
        }
        let aliases = [
        ...
        return try records.map { original in
            ...
            guard let id = record["id"] as? String, UUID(uuidString: id) != nil,
                ...
                throw LocalWorkbenchError.invalid("A legacy message is missing its ID, text, or role. The original file was preserved.")
            }
            if let number = original["sentTime"] as? Double {
                ...
                guard let date else { throw LocalWorkbenchError.invalid("A legacy message has an unreadable timestamp.") }
