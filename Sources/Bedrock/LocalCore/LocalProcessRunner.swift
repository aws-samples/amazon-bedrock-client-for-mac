...
enum LocalProcessRunner {
    ...
    private static func execute(executable: String, arguments: [String], directory: URL,
        ...
        guard FileManager.default.isExecutableFile(atPath: executable) else {
            throw LocalWorkbenchError.unavailable("The command “\(executable)” is not installed or executable.")
        }
        var isDirectory: ObjCBool = false
        guard FileManager.default.fileExists(atPath: directory.path, isDirectory: &isDirectory), isDirectory.boolValue else {
            throw LocalWorkbenchError.unavailable("The working directory is unavailable.")
        }
        let started = Date()
