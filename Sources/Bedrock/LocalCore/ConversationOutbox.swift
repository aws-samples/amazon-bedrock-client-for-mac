...
struct ConversationOutbox: Codable, Equatable, Sendable {
    ...
    mutating func append(_ prompt: QueuedPrompt) throws {
        guard queued.count < 20 else { throw LocalWorkbenchError.invalid("The queue holds up to 20 messages. Send or remove one before adding another.") }
        guard !queued.contains(where: { $0.id == prompt.id }), inFlight?.id != prompt.id else {
            throw LocalWorkbenchError.invalid("This message is already in the queue.")
        }
...
enum ConversationOutboxFile {
    ...
    static func url(threadID: String, directory: URL) throws -> URL {
        guard threadID.range(of: #"^[a-zA-Z0-9_-]{1,128}$"#, options: .regularExpression) != nil else {
            throw LocalWorkbenchError.invalid("This conversation identifier cannot be used for draft storage.")
        }
    ...
    static func read(_ url: URL) throws -> ConversationOutbox {
        ...
        guard properties.isRegularFile == true else { throw LocalWorkbenchError.invalid("The message queue must be a regular file.") }
    ...
    static func validate(_ value: ConversationOutbox) throws {
        ...
        guard value.version == 1, value.queued.count <= 21,
            ...
            throw LocalWorkbenchError.invalid("This saved message queue cannot be opened by this app. Its original file was kept.")
        }
    }
}
