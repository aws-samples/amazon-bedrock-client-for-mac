...
enum LocalArguments {
    ...
    static func parse(_ value: String) throws -> [String] {
        ...
        for character in value {
            guard character != "\0" else { throw LocalWorkbenchError.invalid("Arguments cannot contain NUL characters.") }
        ...
        guard quote == nil, !escaped else { throw LocalWorkbenchError.invalid("Close the quote or escape in the server arguments.") }
...
enum LocalDataMigration {
    ...
    static func copy(from source: URL, to destination: URL) throws {
        ...
        guard source != destination, !destination.path.hasPrefix(source.path + "/"), !source.path.hasPrefix(destination.path + "/") else {
            throw LocalWorkbenchError.invalid("Choose a separate empty folder outside the current data folder.")
        }
        if manager.fileExists(atPath: destination.path),
           !(try manager.contentsOfDirectory(atPath: destination.path)).isEmpty {
            throw LocalWorkbenchError.invalid("Choose an empty folder. Existing destination files will not be overwritten.")
        }
