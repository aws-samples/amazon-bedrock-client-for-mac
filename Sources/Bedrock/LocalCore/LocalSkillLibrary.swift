...
enum LocalSkillLibrary {
    ...
    static func load(_ directory: URL) throws -> LocalSkillLibrarySnapshot {
        ...
        guard children.count <= entryLimit else { throw LocalWorkbenchError.invalid("The skills folder contains too many entries.") }
        ...
        for child in children.sorted(by: { $0.lastPathComponent < $1.lastPathComponent }) {
            ...
            do {
                ...
                guard properties.isSymbolicLink != true else {
                    throw LocalWorkbenchError.invalid("Use an original skill file or folder, not a symbolic link.")
                }
                ...
                try validateID(id)
                guard !ids.contains(id) else { throw LocalWorkbenchError.invalid("Duplicate skill identifier “\(id)”.") }
                let source = try read(url, root: directory)
                bytes += source.utf8.count
                guard bytes <= libraryLimit else { throw LocalWorkbenchError.tooLarge(libraryLimit) }
                let skill = try LocalSkill.parse(source, id: id, url: url)
    ...
    static func read(_ url: URL, root: URL) throws -> String {
        ...
        guard properties.isRegularFile == true, properties.isSymbolicLink != true else {
            throw LocalWorkbenchError.invalid("SKILL.md must be a regular file.")
        }
        ...
        guard let source = String(data: data, encoding: .utf8), !source.contains("\0") else {
            throw LocalWorkbenchError.invalid("SKILL.md must contain UTF-8 text.")
        }
        return source
    }
    ...
    static func save(id: String, source: String, existingURL: URL?, directory: URL) throws {
        ...
        _ = try LocalSkill.parse(source, id: id, url: url)
        ...
        if existingURL == nil, FileManager.default.fileExists(atPath: url.path) {
            throw LocalWorkbenchError.invalid("This skill already exists. Reload and edit its existing instructions.")
        }
    ...
    static func importPackage(from source: URL, into directory: URL) throws -> String {
        ...
        guard properties.isSymbolicLink != true else { throw LocalWorkbenchError.invalid("Choose the original skill instead of a symbolic link.") }
        let folder = properties.isDirectory == true
        ...
        try validateID(id)
        _ = try LocalSkill.parse(raw, id: id, url: file)
        ...
        guard !FileManager.default.fileExists(atPath: target.path),
            ...
            throw LocalWorkbenchError.invalid("A skill named “\(id)” already exists. Rename the imported folder or edit the existing skill.")
        }
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
    ...
    static func references(for skill: LocalSkill) throws -> [LocalFileEntry] {
    ...
    static func export(_ skill: LocalSkill, to destination: URL) throws {
        guard !FileManager.default.fileExists(atPath: destination.path) else {
            throw LocalWorkbenchError.invalid("Choose a new folder name for this exported skill.")
        }
    ...
    private static func copyPackage(from source: URL, into destination: URL) throws {
        ...
        for entry in entries {
            ...
            if entry.isDirectory {
                ...
                bytes += data.count
                guard bytes <= packageLimit else { throw LocalWorkbenchError.tooLarge(packageLimit) }
    ...
    private static func packageEntries(_ root: URL) throws -> [LocalFileEntry] {
        ...
        guard let iterator = FileManager.default.enumerator(at: root, includingPropertiesForKeys: Array(keys),
            ...
            throw LocalWorkbenchError.unavailable("The skill folder cannot be read.")
        }
        ...
        while let url = iterator.nextObject() as? URL {
            ...
            guard properties.isSymbolicLink != true,
                ...
                throw LocalWorkbenchError.invalid("Skill packages must contain regular files and folders without symbolic links.")
            }
            ...
            guard bytes <= packageLimit, result.count < entryLimit else {
                throw LocalWorkbenchError.invalid("Skill packages must be under 50 MB and contain at most \(entryLimit) entries.")
            }
    ...
    private static func validateID(_ id: String) throws {
        guard id.range(of: #"^[a-zA-Z0-9][a-zA-Z0-9_-]{0,79}$"#, options: .regularExpression) != nil else {
            throw LocalWorkbenchError.invalid("Use a skill identifier of letters, numbers, hyphens, or underscores (up to 80 characters).")
        }
    }
}
