...
// MARK: - InferenceSettings
struct InferenceSettings: View {
    @Binding var currentModelId: String
    let backend: BedrockService
...
private struct LumaVideoConfigContent: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            HStack {
                Text("Video settings").font(DesignTokens.label)
                Spacer()
                Button("Reset") {
                    settings.lumaVideoConfig = LumaVideoConfiguration(outputBucket: settings.lumaVideoConfig.outputBucket)
                }.buttonStyle(.borderless).font(DesignTokens.caption)
            }.padding(16)
            Divider()
            ScrollView {
                VStack(alignment: .leading, spacing: 14) {
                    field("Aspect ratio") {
                        SelectionField(title: "Aspect ratio", selection: $settings.lumaVideoConfig.aspectRatio,
                                           options: LumaVideoConfiguration.aspectRatios.map { ($0, $0) })
                    }
                    field("Duration") {
                        AppSegmentedControl(title: "Duration", selection: $settings.lumaVideoConfig.duration,
                                                  options: [("5s", "5 seconds"), ("9s", "9 seconds")])
                    }
                    field("Resolution") {
                        AppSegmentedControl(title: "Resolution", selection: $settings.lumaVideoConfig.resolution,
                    ...
                    Text("Use a bucket in your current AWS region. You can attach a start image and an optional end image.")
                        .font(DesignTokens.detail).foregroundStyle(.secondary).fixedSize(horizontal: false, vertical: true)
                }.padding(16)
            }
        }
...
struct InferenceConfigPopoverContent: View {
    ...
    private var modelName: String {
        ModelCatalog.shared.model(modelId).name
    }
    ...
    private var overrideToggleSection: some View {
        VStack(spacing: 12) {
            HStack {
                ...
                .toggleStyle(AppSwitchStyle(showsLabel: false)).frame(width: 36)
                .controlSize(.small)
    ...
    private var maxTokensControl: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack(spacing: 10) {
                Text("Output limit").font(DesignTokens.label)
                Spacer(minLength: 8)
                ...
                .labelsHidden().toggleStyle(AppSwitchStyle(showsLabel: false)).frame(width: 36)
    ...
    private var topPDisabledControl: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                Text("Top P").font(DesignTokens.label)
                Spacer()
                Text("Omitted").font(DesignTokens.caption).foregroundStyle(.secondary)
            }
    ...
    private var thinkingBudgetControl: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                Text("Thinking budget").font(DesignTokens.label)
                Spacer(minLength: 8)
    ...
    private var streamingControl: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            .font(DesignTokens.label).toggleStyle(AppSwitchStyle())
    ...
    private var reasoningEffortControl: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            AppSegmentedControl(title: "Reasoning effort",
...
struct CustomSlider: View {
    ...
    var body: some View {
        if range.lowerBound < range.upperBound {
            ...
            Text(range.lowerBound.formatted()).font(DesignTokens.detail).foregroundStyle(.secondary)
        }
    }
}
