...
struct DemoLibraryView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 18) {
                PageHeader(title: "Start with a possibility", subtitle: "Editable prompts for your next Bedrock demo. Choose one, make it yours, then send.")
                HStack {
                ...
                SelectionField(title: "Category", selection: $category,
                ...
                if filtered.isEmpty { EmptyStateView(symbol: "magnifyingglass", title: "No matching demos", detail: "Try another search or create a prompt of your own.") }
            }
            .padding(32).frame(maxWidth: 1120).frame(maxWidth: .infinity)
        }
        .sheet(item: $editing) { demo in DemoEditor(demo: demo) }
    ...
    private func card(_ demo: DemoPreset) -> some View {
        VStack(alignment: .leading, spacing: 15) {
            HStack {
                Image(systemName: demo.category.symbol).font(.system(size: 20, weight: .light)).foregroundStyle(DesignTokens.accent)
                Spacer()
                ActionMenu {
                    Button("Duplicate") {
                        var copy = demo
                    ...
                    Button("Export…") { AppActions.exportDemo(demo) }
                    if !demo.isBuiltIn {
                        Divider()
        ...
        .padding(20)
        .background(DesignTokens.surface.opacity(0.55), in: RoundedRectangle(cornerRadius: 14))
    ...
    private func importDemo() {
        ...
        panel.begin { result in
            ...
            do {
                ...
                guard !demo.title.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty, !demo.prompt.isEmpty else { throw LocalOperationError.invalid("The demo needs a title and prompt.") }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            Form {
                ...
                SelectionField(title: "Category", selection: $demo.category,
                ...
                TextEditor(text: $demo.prompt).font(.system(size: 13)).frame(minHeight: 190).border(DesignTokens.border)
                DisclosureGroup("System instructions") { TextEditor(text: $demo.systemPrompt).font(.system(size: 12)).frame(height: 90) }
            }
            HStack {
                Button("Cancel", role: .cancel) { dismiss() }.keyboardShortcut(.cancelAction)
                Spacer()
                Button("Save demo") { AppStore.shared.saveDemo(demo); dismiss() }
...
struct DemoVariablesSheet: View {
    let demo: DemoPreset
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 18) {
            ...
            HStack {
                ...
                Spacer()
                Button("Prepare prompt", action: use).buttonStyle(AppButtonStyle(prominent: true)).keyboardShortcut(.defaultAction)
            }
        }
...
struct ModelComparisonSheet: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 18) {
            ...
            TextEditor(text: $prompt).font(.system(size: 13)).frame(height: 180).border(DesignTokens.border)
            HStack {
                Button("Cancel") { dismiss() }.keyboardShortcut(.cancelAction)
                Spacer()
        ...
        .onAppear {
            first = ModelCatalog.shared.defaultModel?.id ?? models.first?.id ?? ""
    ...
    private func modelPicker(_ label: String, selection: Binding<String>) -> some View {
        SelectionField(title: label, selection: selection,
    ...
    private func run() {
        ...
        let firstChat = AppActions.newThread(model: a, draft: prompt)
        let secondChat = AppActions.newThread(model: b, draft: prompt, select: false)
        AppStore.shared.companionThreadID = secondChat.chatId
