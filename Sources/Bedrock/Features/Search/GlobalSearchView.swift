...
struct GlobalSearchView: View {
    ...
    private enum Target {
        case newThread, importThread, setting(String), page(NavigationDestination), thread(String), demo(DemoPreset), skill(String)
    }
    ...
    private var items: [Item] {
        ...
        for destination in NavigationDestination.allCases where destination != .chats {
    ...
    var body: some View {
        ...
        VStack(spacing: 0) {
            ...
            Divider()
            if rows.isEmpty {
                EmptyStateView(symbol: "magnifyingglass", title: isSearching ? "Searching conversations…" : "No results",
                                ...
                                Button { activate(item) } label: {
                                    HStack(spacing: 11) {
                                        ...
                                        VStack(alignment: .leading, spacing: 3) {
                                            Text(item.title).font(DesignTokens.label).lineLimit(1)
                                            Text(item.detail).font(DesignTokens.detail).foregroundStyle(.secondary).lineLimit(1)
        ...
        .task {
            await Task.yield()
            focused = true
            EditorFocusState.shared.isSearchFieldActive = true
        }
        .onDisappear { EditorFocusState.shared.isSearchFieldActive = false }
    ...
    private func activate(_ item: Item) {
        ...
        Task { @MainActor in
            ...
            case .importThread: AppActions.importThread()
