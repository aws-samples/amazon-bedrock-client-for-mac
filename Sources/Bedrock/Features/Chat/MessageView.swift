...
extension NSImage {
    ...
    convenience init?(base64Encoded: String) {
        ...
        if base64Encoded.hasPrefix("img_") {
            // Read defaultDirectory from UserDefaults (same key as @AppStorage in PreferencesStore)
            // This avoids MainActor issues while staying in sync with PreferencesStore
