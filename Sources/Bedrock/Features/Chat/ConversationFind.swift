...
struct SearchMatch: Equatable {
    ...
    let snippet: String
    let score: Double
    var selectedRangeIndex: Int? = nil
    ...
    static func == (lhs: SearchMatch, rhs: SearchMatch) -> Bool {
        return lhs.messageIndex == rhs.messageIndex &&
               ...
               lhs.score == rhs.score &&
               lhs.selectedRangeIndex == rhs.selectedRangeIndex
    }
}
...
class SearchEngine: ObservableObject {
    ...
    private func findMatches(searchTerms: [String], in text: String) -> [NSRange] {
        ...
        for term in searchTerms {
            let ranges = findAllOccurrences(of: term, in: text)
    ...
    private func generateSnippet(text: String, ranges: [NSRange], maxLength: Int) -> String {
        ...
        let snippetEnd = min(source.length, NSMaxRange(firstRange) + maxLength / 2)
        let range = source.rangeOfComposedCharacterSequences(for: NSRange(location: snippetStart, length: snippetEnd - snippetStart))
        var snippet = source.substring(with: range)
        
        // Add ellipsis if truncated
        if range.location > 0 {
            snippet = "..." + snippet
        }
        if NSMaxRange(range) < source.length {
            snippet = snippet + "..."
        }
    ...
    private func tokenizeQuery(_ query: String) -> [String] {
        // Find in a conversation follows the native exact-phrase convention.
        // Splitting a phrase also made the counter disagree with its highlights.
        [query]
    }
    ...
    static func createHighlightedText(
        ...
        if #available(macOS 12.0, *) {
            ...
            for (index, range) in searchRanges.enumerated() {
                ...
                if let start = AttributedString.Index(stringRange.lowerBound, within: attributed),
