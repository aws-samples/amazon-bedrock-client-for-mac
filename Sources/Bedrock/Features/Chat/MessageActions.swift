...
struct MessageEditor: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            TextEditor(text: $text).font(DesignTokens.body).scrollContentBackground(.hidden)
                .padding(12).background(DesignTokens.field, in: RoundedRectangle(cornerRadius: 10))
            ...
            if let error { Text(error).font(DesignTokens.caption).foregroundStyle(.orange).textSelection(.enabled) }
            HStack {
        ...
        .padding(24).frame(width: 580).background(DesignTokens.canvas)
...
struct MessageQueueView: View {
    ...
    var body: some View {
                        ...
                        HStack(spacing: 8) {
                            ...
                            Button { editing = prompt } label: {
                                VStack(alignment: .leading, spacing: 3) {
                                    ...
                                    HStack(spacing: 6) {
                                        Text(ModelCatalog.shared.model(prompt.modelID).name)
        ...
        .animation(reduceMotion ? nil : AppMotion.standard, value: prompts.map(\.id))
        .animation(reduceMotion ? nil : AppMotion.standard, value: expanded)
        ...
        .sheet(item: $editing) { prompt in
            ...
            MessageEditor(message: message, title: "Queued message",
...
struct MessageDetails: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 18) {
            ...
            Grid(alignment: .leading, horizontalSpacing: 20, verticalSpacing: 14) {
                ...
                GridRow {
                    ...
                    Text(ModelCatalog.shared.model(modelID).name)
                }
                GridRow {
        ...
        }.padding(24).frame(width: 540).background(DesignTokens.canvas)
    }
}

struct PastedTextEditor: View {
    let filename: String
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            Text(filename).font(DesignTokens.caption).foregroundStyle(.secondary)
        ...
        }.padding(24).frame(width: 600).background(DesignTokens.canvas)
