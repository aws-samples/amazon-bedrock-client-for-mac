...
@MainActor
enum LocalToolManager {
    static func availableTools(threadID: String) -> [LocalToolKind] {
        let enabled = WorkbenchStore.shared.preferences.enabledTools
        return LocalToolKind.allCases.filter { enabled.contains($0) }
    }
    ...
    static func authorize(name: String, input: JSONValue, threadID: String) async -> Bool {
        ...
        let kind = LocalToolKind(rawValue: name)
        ...
        let tool = MCPManager.shared.toolInfo(named: name)
    ...
    static func execute(kind: LocalToolKind, input: JSONValue, threadID: String, modelID: String) async -> ChatViewModel.SendableToolResult {
        let store = WorkbenchStore.shared
        ...
        func string(_ key: String) throws -> String {
            guard let value = values[key] as? String, !value.isEmpty else {
                throw LocalWorkbenchError.invalid("Missing required tool input: \(key).")
            }
            return value
        }
        do {
            ...
            case .writeFile:
                let path = try string("path")
                guard let content = values["content"] as? String else { throw LocalWorkbenchError.invalid("Missing file content.") }
                output = try await fileWork {
                    let location = try access.resolve(path, allowRoot: false)
            ...
            case .runCommand, .gitStatus:
                ...
                if kind == .runCommand {
                    ...
                    default: throw LocalWorkbenchError.invalid("Git action must be status, diff, or log.")
                    }
                }
            ...
            case .openURL:
                let url = try LocalPath.validatedWebURL(try string("url"), allowedDomains: preferences.allowedWebDomains)
                guard NSWorkspace.shared.open(url) else { throw LocalWorkbenchError.unavailable("macOS could not open this URL.") }
            ...
            case .readSkill:
                ...
                guard let skill = store.skills.first(where: { $0.id == id }), store.isSkillEnabled(skill) else {
                    throw LocalWorkbenchError.unavailable("This skill is not enabled. Use local_list_skills for available IDs.")
                }
                if let reason = try await fileWork({ skill.unavailableReason() }) { throw LocalWorkbenchError.unavailable(reason) }
...
private final class DirectWebFetcher: NSObject, URLSessionTaskDelegate, @unchecked Sendable {
    ...
    static func fetch(url: URL, allowedDomains: String, limit: Int, timeout: Int) async throws -> String {
        ...
        guard let http = response as? HTTPURLResponse, (200..<300).contains(http.statusCode) else {
            throw LocalWorkbenchError.unavailable("The web request returned HTTP \((response as? HTTPURLResponse)?.statusCode ?? 0).")
        }
        var data = Data()
        ...
        guard let text = String(data: data, encoding: .utf8), !text.contains("\0") else {
            throw LocalWorkbenchError.invalid("This URL returned binary content rather than UTF-8 text.")
        }
