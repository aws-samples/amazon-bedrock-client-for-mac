...
class Backend: Equatable, @unchecked Sendable {
            ...
            for (index, content) in (message.content ?? []).enumerated() {
                ...
                case .tooluse(let tool):
                    guard let id = tool.toolUseId, let name = tool.name, let input = tool.input else {
                        throw LocalWorkbenchError.invalid("The model returned an incomplete tool call.")
                    }
    ...
    func isMantleModelAvailableInRegion(_ modelId: String) -> Bool {
        guard let regions = Backend.mantleRegions(for: modelId) else { return true }
    ...
    func makeConverseRequest(
        ...
        if await MainActor.run(body: { WorkbenchStore.shared.preferences.promptCaching }) && isPromptCachingSupported(modelId) && !messages.isEmpty {
    ...
    func converseStream(
        ...
        let useStreaming = await MainActor.run {
            let config = SettingManager.shared.getInferenceConfig(for: modelId)
    ...
    func mantleResponsesStream(
        ...
        if let regions = Backend.mantleRegions(for: modelId), !regions.contains(region) {
        ...
        do {
            ...
            repeat {
                ...
                if let token, !seenTokens.insert(token).inserted {
                    throw LocalWorkbenchError.invalid("Bedrock repeated a model catalog page token.")
                }
            } while token != nil
