...
class MCPManager: ObservableObject {
    ...
    private var processes: [String: WorkbenchMCPProcess] = [:]
    ...
    func loadConfigFile() {
        ...
        do {
            let loaded = try WorkbenchMCPConfiguration.decode(Data(contentsOf: url))
            servers = loaded
            configLoadFailed = false
        } catch {
            ...
            DispatchQueue.main.async {
                WorkbenchStore.shared.errorMessage = "Could not read mcp_config.json. The original file was preserved. Correct it and reopen the app.\n\(error.localizedDescription)"
            }
        }
    }
    ...
    func saveConfigFile() {
        ...
        do {
            let data = try WorkbenchMCPConfiguration.encode(servers)
            let url = URL(fileURLWithPath: getConfigPath())
            ...
            DispatchQueue.main.async {
                WorkbenchStore.shared.errorMessage = "Could not save MCP configuration: \(error.localizedDescription)"
            }
        }
    }
    ...
    func connectToServer(_ server: MCPServerConfig) {
        ...
        connectionTasks[server.name] = Task {
            ...
            var process: WorkbenchMCPProcess?
            do {
                try WorkbenchMCPConfiguration.validate(server)
                let transport: any Transport
                switch server.transportType {
                case .stdio:
                    let child = try WorkbenchMCPProcess(command: server.command, arguments: server.args,
                ...
                let tools: [Tool] = try await Self.withTimeout(seconds: connectionTimeout, interrupt: { await client.disconnect() }) {
                    ...
                    repeat {
                        ...
                        guard result.count <= 2_000 else { throw LocalWorkbenchError.invalid("This MCP server returned too many tools.") }
                        cursor = page.nextCursor
                        if let cursor, !seen.insert(cursor).inserted {
                            throw LocalWorkbenchError.invalid("This MCP server repeated a tool-list cursor.")
                        }
                    } while cursor != nil
                    return result
                }
                ...
                if error is CancellationError || Task.isCancelled { connectionStatus[server.name] = .notConnected }
                    ...
                    connectionStatus[server.name] = .failed(error: error.localizedDescription + (detail.isEmpty ? "" : "\n" + detail))
        ...
        try await withTaskCancellationHandler {
            try await withThrowingTaskGroup(of: T.self) { group in
                ...
                group.addTask {
                    ...
                    await interrupt()
                    throw LocalWorkbenchError.unavailable("The MCP request timed out. Reconnect the server and try again.")
                }
    ...
    nonisolated private static func encodeResult(_ result: CallTool.Result) throws -> Data {
        ...
        guard data.count <= 32_000_000 else { throw LocalWorkbenchError.tooLarge(32_000_000) }
        try Task.checkCancellation()
        return data
    }
        ...
        do {
            ...
            if let info = toolInfo(named: name), let client = activeClients[info.serverName] {
                ...
                guard var output = try JSONSerialization.jsonObject(with: data) as? [String: Any] else {
                    throw LocalWorkbenchError.invalid("The MCP server returned an invalid tool result.")
                }
                output["id"] = toolId
