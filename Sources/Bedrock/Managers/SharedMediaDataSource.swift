...
class SharedMediaDataSource: ObservableObject {
    ...
    func copy(from source: SharedMediaDataSource) {
        cancelImports()
    ...
    func attachmentDraft() throws -> ConversationAttachmentDraft {
        guard images.count == imageIDs.count, images.count == imageEncodedData.count,
            ...
            throw LocalWorkbenchError.invalid("The attachment list is still being prepared.")
        }
        let imageItems = try images.indices.map { index in
            guard let data = imageEncodedData[index] else {
                throw LocalWorkbenchError.invalid("An image has not finished preparing for storage.")
            }
    ...
    func restore(_ message: MessageData, imagesDirectory directory: URL) async throws {
        ...
        let worker = Task.detached(priority: .userInitiated) {
            ...
            for (index, value) in (message.documentBase64Strings ?? []).enumerated() {
                guard let data = Data(base64Encoded: value) else { throw LocalWorkbenchError.invalid("The document attachment could not be decoded.") }
            ...
            for url in urls.prefix(25) {
                ...
                do {
                    ...
                    case .image(let image, let filename):
                        guard self.images.count < 20 else { throw LocalWorkbenchError.invalid("A message supports up to 20 images.") }
                        guard self.addPreparedImage(image, filename: filename) else {
                            throw LocalWorkbenchError.invalid("The image preview could not be opened.")
                        }
                    case .document(let data, let ext, let filename):
                        guard self.documents.count < 5 else { throw LocalWorkbenchError.invalid("A message supports up to 5 documents.") }
