...
enum LocalAttachmentProcessor {
    ...
    static func prepare(_ url: URL) async throws -> PreparedLocalAttachment {
        ...
        guard url.isFileURL else { throw LocalWorkbenchError.invalid("Choose a local file.") }
        ...
        guard documentInputExtensions.contains(ext) else {
            throw LocalWorkbenchError.invalid("The file type .\(ext) is not supported as an attachment.")
        }
        let values = try url.resourceValues(forKeys: [.fileSizeKey, .isRegularFileKey])
        guard values.isRegularFile == true else { throw LocalWorkbenchError.invalid("Choose a regular document file.") }
        guard (values.fileSize ?? 0) <= maximumDocumentBytes else { throw LocalWorkbenchError.tooLarge(maximumDocumentBytes) }
        let handle = try FileHandle(forReadingFrom: url)
        defer { try? handle.close() }
        let data = try handle.read(upToCount: maximumDocumentBytes + 1) ?? Data()
        guard data.count <= maximumDocumentBytes else { throw LocalWorkbenchError.tooLarge(maximumDocumentBytes) }
        try Task.checkCancellation()
        if isSource {
            guard !data.contains(0), String(data: data, encoding: .utf8) != nil else {
                throw LocalWorkbenchError.invalid("This source file must contain UTF-8 text.")
            }
        }
...
enum ClipboardImageProcessor {
    ...
    static func prepare(_ input: ClipboardImageInput) async throws -> PreparedClipboardImage {
        ...
        case .file(let url):
            guard url.isFileURL else { throw LocalWorkbenchError.invalid("Choose a local image file.") }
            let values = try url.resourceValues(forKeys: [.fileSizeKey, .isRegularFileKey])
            guard values.isRegularFile == true else { throw LocalWorkbenchError.invalid("Choose a regular image file.") }
            guard (values.fileSize ?? 0) <= maximumInputBytes else { throw LocalWorkbenchError.tooLarge(maximumInputBytes) }
            let handle = try FileHandle(forReadingFrom: url)
            defer { try? handle.close() }
            data = try handle.read(upToCount: maximumInputBytes + 1) ?? Data()
        case .source(let source):
            guard ClipboardHTMLParser.isImageSourceAllowed(source) else {
                throw LocalWorkbenchError.invalid("This clipboard image source is unsupported.")
            }
            if source.lowercased().hasPrefix("data:") {
                guard source.utf8.count <= ClipboardHTMLParser.maximumHTMLBytes,
                    ...
                    throw LocalWorkbenchError.invalid("The clipboard image data is invalid.")
                }
                data = decoded
            } else {
    ...
    static func decode(_ data: Data) throws -> PreparedClipboardImage {
        guard data.count <= maximumInputBytes else { throw LocalWorkbenchError.tooLarge(maximumInputBytes) }
        guard let source = CGImageSourceCreateWithData(data as CFData, [kCGImageSourceShouldCache: false] as CFDictionary),
            ...
            throw LocalWorkbenchError.invalid("This image could not be decoded within the supported dimensions.")
        }
        ...
        while dimension > 0 {
            ...
            guard let image = CGImageSourceCreateThumbnailAtIndex(source, 0, options as CFDictionary),
                ...
                throw LocalWorkbenchError.invalid("This image could not be prepared for the message.")
            }
        ...
        guard let encoded else { throw LocalWorkbenchError.tooLarge(maximumOutputBytes) }
        ...
        guard let previewImage = CGImageSourceCreateThumbnailAtIndex(source, 0, previewOptions as CFDictionary),
            ...
            throw LocalWorkbenchError.invalid("This image preview could not be prepared.")
        }
...
private final class ClipboardImageDownload: NSObject, URLSessionDataDelegate, @unchecked Sendable {
    ...
    func urlSession(_ session: URLSession, dataTask: URLSessionDataTask, didReceive response: URLResponse,
        ...
        guard let http = response as? HTTPURLResponse, (200..<300).contains(http.statusCode),
            ...
            finish(.failure(LocalWorkbenchError.invalid("The copied image URL did not return a supported image.")))
            return
        }
    ...
    func urlSession(_ session: URLSession, task: URLSessionTask, willPerformHTTPRedirection response: HTTPURLResponse,
        ...
        guard let value = request.url?.absoluteString, (try? LocalPath.validatedWebURL(value, allowedDomains: "")) != nil else {
            ...
            finish(.failure(LocalWorkbenchError.invalid("The image URL redirected to an unsupported address.")))
            return
        }
    ...
    func urlSession(_ session: URLSession, dataTask: URLSessionDataTask, didReceive chunk: Data) {
        ...
        guard data.count + chunk.count <= Self.limit else {
            lock.unlock()
            finish(.failure(LocalWorkbenchError.tooLarge(Self.limit)))
            return
        }
        data.append(chunk)
        lock.unlock()
