...
actor WorkbenchImageIO {
    static let shared = WorkbenchImageIO()
    ...
    func load(_ input: ImagePreviewSource, maximumPixelSize: Int = 2_560) throws -> PreparedImagePreview {
        ...
        case .stored(let reference, let directory):
            guard reference.utf8.count <= Self.maximumInputBytes * 4 / 3 + 4 else {
                throw LocalWorkbenchError.tooLarge(Self.maximumInputBytes)
            }
        ...
        case .legacy(let bitmap):
            guard let encoded = bitmap.image.tiffRepresentation else {
                throw LocalWorkbenchError.invalid("The image could not be opened.")
            }
            data = encoded
        }
        guard !data.isEmpty else { throw LocalWorkbenchError.invalid("The image is empty.") }
        guard data.count <= Self.maximumInputBytes else { throw LocalWorkbenchError.tooLarge(Self.maximumInputBytes) }
        try Task.checkCancellation()
    ...
    nonisolated static func decode(_ data: Data, maximumPixelSize: Int) throws -> PreparedImagePreview {
        guard data.count <= maximumInputBytes,
            ...
            throw LocalWorkbenchError.invalid("This image is damaged or exceeds the supported dimensions.")
        }
        try Task.checkCancellation()
        ...
        guard let preview = CGImageSourceCreateThumbnailAtIndex(source, 0, options as CFDictionary) else {
            throw LocalWorkbenchError.invalid("The image preview could not be decoded.")
        }
    ...
    func export(_ image: PreparedImagePreview, as type: UTType) throws -> Data {
        ...
        guard let source = CGImageSourceCreateWithData(image.original as CFData, [kCGImageSourceShouldCache: false] as CFDictionary),
            ...
            throw LocalWorkbenchError.invalid("The original image could not be exported.")
        }
        try Task.checkCancellation()
        let data = NSMutableData()
        guard let destination = CGImageDestinationCreateWithData(data, type.identifier as CFString, 1, nil) else {
            throw LocalWorkbenchError.invalid("This image format is not supported for export.")
        }
        CGImageDestinationAddImage(destination, fullSize, [kCGImageDestinationLossyCompressionQuality: 0.95] as CFDictionary)
        guard CGImageDestinationFinalize(destination) else { throw LocalWorkbenchError.invalid("The image could not be encoded.") }
        try Task.checkCancellation()
        return data as Data
    }
