...
@MainActor
final class WorkbenchAutomationRunner: ObservableObject {
    static let shared = WorkbenchAutomationRunner()
    @Published private(set) var running: [UUID: String] = [:]
    private var tasks: [UUID: Task<Void, Never>] = [:]
    private var ticker: Task<Void, Never>?
    private var backend: BackendModel?

    func start(backend: BackendModel) {
    ...
    func save(_ automation: LocalAutomation) throws {
        if let error = automation.validationError { throw LocalWorkbenchError.invalid(error) }
    ...
    func runNow(_ automation: LocalAutomation) {
        ...
        tasks[automation.id] = Task { [weak self] in
            ...
            let store = WorkbenchStore.shared
            let model = WorkbenchModelCatalog.shared.models.first { $0.id == automation.modelID } ??
            ...
            let chat = WorkbenchActions.newThread(model: model, draft: automation.prompt, workingDirectory: automation.workingDirectory,
                                                  skillIDs: automation.skillIDs, select: false)
            ChatManager.shared.updateChatTitle(for: chat.chatId, title: automation.name, isManualRename: true)
