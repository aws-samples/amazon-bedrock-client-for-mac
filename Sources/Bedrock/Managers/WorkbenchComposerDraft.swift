...
@MainActor
final class WorkbenchComposerDraft: ObservableObject {
    static let welcome = WorkbenchComposerDraft()
    ...
    private func restore() async {
        ...
        do {
            let url = try ConversationAttachmentDraftFile.url(threadID: threadID, directory: WorkbenchStore.shared.directory)
            ...
            restored = true
            WorkbenchStore.shared.updateThread(threadID) { $0.hasDraftAttachments = !media.isEmpty }
            if changedBeforeRestore { changed() }
        } catch {
            restored = true
            canWrite = false
            WorkbenchStore.shared.errorMessage = "Could not restore the new-chat attachments. The saved draft was kept.\n\(error.localizedDescription)"
        }
    }
    ...
    private func persist() async -> Bool {
        ...
        do {
            ...
            WorkbenchStore.shared.errorMessage = "Could not save the new-chat attachments. Keep the app open and check the data folder.\n\(error.localizedDescription)"
            return false
        }
    }
