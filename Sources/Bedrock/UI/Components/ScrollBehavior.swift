...
struct ConversationScrollBehavior: ViewModifier {
    ...
    func body(content: Content) -> some View {
        if #available(macOS 15.0, *) {
            ...
            content.background(LegacyScrollObserver { nearBottom, ended in
                userDidScroll()
                if followsOutput != nearBottom { followsOutput = nearBottom }
...
private struct LegacyScrollObserver: NSViewRepresentable {
