...
If a freeze persists, record the app version, macOS version, approximate text/image sizes, and the action that triggered it. A redacted or synthetic reproduction is more useful than a screenshot alone. Development performance checks use an optimized Release build; see [the validation procedure](docs/DEVELOPMENT.md#performance-checks).
