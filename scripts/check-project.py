...
def main():
    ...
    expected = {
        "Bedrock": set((root / "Sources/Bedrock").rglob("*.swift")),
        "BedrockTests": set((root / "Tests/BedrockTests").glob("*.swift")),
        "BedrockUITests": set((root / "Tests/BedrockUITests").glob("*.swift")),
    }
    imported_modules = {
        module
        for path in expected["Bedrock"]
